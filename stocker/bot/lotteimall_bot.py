...
class LotteIMallBot(Bot):
    site_name = 'LOTTEIMALL'

    def __init__(self, config, output):
        super().__init__(config, output)
        lotteimall_config = self.site_config[self.site_name]
        account_config = lotteimall_config['ACCOUNT']
        self.user = account_config['USER']
        self.password = account_config['PASSWORD']
        urls = lotteimall_config['URL']
        self.base_url = urls['BASE']
        self.product_url = urls['PRODUCT']
        pay_config = lotteimall_config['PAY']
